...
export async function handleIssueEvent(payload: unknown): Promise<void> {
  ...
  console.log(
    `[Issue Handler] Processing ${action} for issue #${issue.number}`,
  );
  ...
  try {
    ...
    if (!repo) {
      console.warn(
        `[Issue Handler] Repository not found: ${repository.full_name}`,
      );
      return;
    }
    ...
    switch (action) {
      ...
      case "unassigned":
        ...
        if (assignError) {
          console.error(
            "[Issue Handler] Failed to update assignee:",
            assignError,
          );
          return;
        }
    ...
    console.log(
      `[Issue Handler] Successfully processed ${action} for issue #${issue.number}`,
    );
  } catch (error) {
